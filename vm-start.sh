...
# Initialize an array to keep track of VMs that failed to provision
FAILED_VMS=()
...
do
    ...
    if [ "$VM_PROVISIONING_STATE" == "Succeeded" ]; then
        echo -e "\033[36mVM $VM_NAME is successfully provisioned.\033[0m"  # Cyan color for success
    else
        echo -e "\033[31mVM $VM_NAME is NOT successfully provisioned. Current provisioning state: $VM_PROVISIONING_STATE\033[0m"
...
if [ ${#FAILED_VMS[@]} -ne 0 ]; then
    echo -e "\033[31mThe following VMs failed to provision:\033[0m"
    for VM in "${FAILED_VMS[@]}"
    do
        echo -e "\033[31m - $VM\033[0m"
    done
else
    echo -e "\033[32mAll VMs are successfully provisioned.\033[0m"  # Green color for all successful
fi